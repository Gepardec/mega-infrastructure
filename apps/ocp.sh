...
function createMegaSecrets {
    oc create secret generic mega --from-file=filename=../secrets/mega/mega-secrets.${STAGE}.properties
    oc create secret generic mega-zep-db --from-env-file=../secrets/mega-db/mega-zep-db.${STAGE}.properties
}
...
function createMegaBackupPvc {
    oc apply -f backup/mega-backup-pvc.yaml
}

function deleteMegaBackupPvc {
