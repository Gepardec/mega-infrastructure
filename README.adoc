...
IMPORTANT: The backups are synced to a local machine periodically and uploaded to the secure storage manually because the storage cannot be accessed by Jenkins directly.

=== Download backups

The backup files ca be synced to a local machine via the Openshift client binary.

.Sync Backups to local machine
[source,bash]
----
oc rsync <POD_ID>:/mega-backups/ .
----
